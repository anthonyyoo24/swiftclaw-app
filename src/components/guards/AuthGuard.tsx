...
export function AuthGuard({ children }: { children: React.ReactNode }) {
    ...
    useEffect(() => {
        if (status === "incomplete" || setupStatus === "missing") {
            if (setupStatus === "missing") clearOnboardingCompleteCookie();
            router.replace("/onboarding");
        }
    }, [status, setupStatus, router]);

    if (status !== "complete" || setupStatus !== "configured" || isAuthLoading || !isAuthenticated) {
        return (
            <div
