 ...
 *
 * Progress is driven entirely by backend IPC events via `currentStep` / `totalSteps`.
 * When `backendComplete` is true, triggers a 250ms animation to 100% then calls
 * `onVisualComplete` so the parent can transition to the success view.
 */
export function DeployProgressView({ currentStep, totalSteps, progressLabel, backendComplete, onVisualComplete }: DeployProgressViewProps) {
    ...
    // Trigger completion animation when backend signals done
    useEffect(() => {
        if (backendComplete && !completing) {
            setCompleting(true);
        }
    }, [backendComplete, completing]);
    ...
    const rawPercent = completing ? 100 : Math.min((currentStep / totalSteps) * 100, 100);
    const transitionDuration = completing ? COMPLETION_DURATION_MS : 300;
